...
namespace ws
{
	...
	//返回字符串实际长度（单位是一个英文字符）
	const unsigned getStringLength ( const std::string & str, const std::string & coding = "utf-8" )
	{
		unsigned result = 0;
		if (coding == "utf-8")
		{
    ...
    namespace _strFormat
    {
		...
		std::string scientificToDefault ( const std::string & source )
		{
		...
		bool _format_D ( std::stringstream & p_strs, const std::string & p_format, const std::string & p_source )
		{
		...
		bool _format_F ( std::stringstream & p_strs, const std::string & p_format, const std::string & p_source )
		{
		...
		bool _format_C ( std::stringstream & p_strs, const std::string & p_format, const std::string & p_source )
		{
		...
		bool _format_aline ( std::stringstream & p_strs, const std::string & p_format, const std::string & p_source )
		{
			...
			if (matchBegin->str ( 1 ).c_str () == std::string ( "-" ))
			{
				for (int i = (int)getStringLength ( p_source ); i < minCount; ++i)
				{
					p_strs << " ";
				}
			}
...
#undef RETURN
        ...
        template<typename Argument>
		std::string & replace( std::string & p_str, const std::vector<placeholder> & p_placeholders, const unsigned p_parameterIndex, const Argument & p_arg )
        {
		...
		std::string & replace( std::string & p_str, const std::vector<placeholder> & p_placeholders, const unsigned p_parameterCount, const Argument & p_arg, const Arguments & ... p_args )
        {
